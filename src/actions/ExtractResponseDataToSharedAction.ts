...
@injectable()
export class ExtractResponseDataToSharedAction extends AbstractAction {

  name = 'ExtractResponseDataToShared';
  ...
  async execute(context: ActionExecutionContext): Promise<{ next: boolean, result: any }> {
    ...
    await dispatch.app.changeSharedStateBulk({ items: changeValues });
